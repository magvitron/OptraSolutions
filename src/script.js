...
const normalTexture = textureloader.load('/textures/world.png');
// Debug panels
const gui = new dat.GUI();

// Canvas
const canvas = document.querySelector('canvas.webgl');
const canvasx = document.querySelector('canvas.webgl');
// Scene
...
for (let count =0;count <1 ;count ++)
{
    const Picmaterial = new THREE.MeshStandardMaterial({
        map:textureloader.load(`/projects/${count}.png`)
    })
    //creating a mesh
    const image = new THREE.Mesh(planeGeometry,Picmaterial);
    image.position.set(1,count*1.8);
    // scene.add(image );
}
